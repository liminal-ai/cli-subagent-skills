...
async function main() {
  ...
  const sourceDir = join(repoRoot, "skills", "claude-subagent");
  const targetRoot = join(homedir(), ".claude", "skills");
  const targetDir = join(targetRoot, "claude-subagent");
  ...
  const deployedScript = join(targetDir, "scripts", "claude-result");
