...
async function main() {
  ...
  const sourceDir = join(repoRoot, "skills", "cursor-subagent");
  const targetRoot = join(homedir(), ".claude", "skills");
  const targetDir = join(targetRoot, "cursor-subagent");
  ...
  const deployedScript = join(targetDir, "scripts", "cursor-result");
