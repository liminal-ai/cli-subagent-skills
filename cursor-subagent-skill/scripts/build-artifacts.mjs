...
async function copyDirectoryFiltered(src, dest) {
...
async function main() {
  ...
  const skillSourceDir = join(repoRoot, "skills", "cursor-subagent");
  ...
  const artifactBase = `cursor-subagent-v${version}`;
  ...
  const tempRoot = await fs.mkdtemp(join(tmpdir(), "cursor-subagent-skill-"));

  try {
    const stagedSkillDir = join(tempRoot, "cursor-subagent");
    await copyDirectoryFiltered(skillSourceDir, stagedSkillDir);

    const zipResult = spawnSync("zip", ["-r", "-q", zipPath, "cursor-subagent"], {
      cwd: tempRoot,
      encoding: "utf8",
    });
