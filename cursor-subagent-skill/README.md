...
Status: alpha and unstable. Cursor CLI currently shows anomalies in subagent workflows, so this skill is not recommended for normal use.
...
- build a versioned `.zip` plus an expanded `dist/cursor-subagent/` directory
...
- `dist/cursor-subagent/` mirrors the packaged skill directory for local inspection and install testing.
