...
async function main(): Promise<void> {
  ...
  const skillSourceDir = join(repoRoot, "skills", "codex-subagent");
  ...
  const artifactBase = `codex-subagent-v${version}`;
  ...
  const tempRoot = await fs.mkdtemp(join(tmpdir(), "codex-subagent-skill-"));

  try {
    const stagedSkillDir = join(tempRoot, "codex-subagent");
    await copyDirectoryFiltered(skillSourceDir, stagedSkillDir);

    const zipResult = spawnSync("zip", ["-r", "-q", zipPath, "codex-subagent"], {
      cwd: tempRoot,
      encoding: "utf8",
    });
