...
async function main(): Promise<void> {
  ...
  const sourceDir = join(repoRoot, "skills", "codex-subagent");
  const targetRoot = join(homedir(), ".claude", "skills");
  const targetDir = join(targetRoot, "codex-subagent");
  ...
  const deployedScript = join(targetDir, "scripts", "codex-result");
